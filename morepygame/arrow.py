...
def _compute_left_right(surface, start, end, head_length=None, head_angle_rad=None):
    ...
    start, end = mp.scr2phy((start, end), surface)
    ...
    head_angle_rad = mp.rad2deg(head_angle_rad) if head_angle_rad else 30
    left_vec = pointe.rotate(-head_angle_rad)
    right_vec = pointe.rotate(head_angle_rad)
    left = end + left_vec
    right = end + right_vec
    left, right = mp.phy2scr((left, right), surface)
    return left, right
...
def draw_arrow_by_angle_rad(
    ...
    """
    start = mp.scr2phy(start, surface)
